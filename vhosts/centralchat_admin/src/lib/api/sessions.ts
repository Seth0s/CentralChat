...
export const getAdminSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) =>
    orchestratorJson<{ session: ChatSession & { messages?: { role: string; content: string }[] }; ok: boolean }>(
      `/admin/sessions/${data.id}`,
    ),
  );
...
export const fetchSessionAcl = createServerFn({ method: "POST" })
  .inputValidator(z.object({ sessionId: z.string() }))
  .handler(async ({ data }) =>
    orchestratorJson<{ items: SessionAclEntry[]; count: number }>(
      `/admin/sessions/${data.sessionId}/acl`,
    ),
  );

export const upsertSessionAcl = createServerFn({ method: "POST" })
  .inputValidator(
    z.object({
      sessionId: z.string(),
  ...
  .handler(async ({ data }) =>
    orchestratorJson<{ entry: SessionAclEntry; ok: boolean }>(
      `/admin/sessions/${data.sessionId}/acl`,
      {
        method: "PUT",
        body: JSON.stringify({
...
export const deleteSessionAcl = createServerFn({ method: "POST" })
  .inputValidator(
    z.object({
      sessionId: z.string(),
      principalType: z.enum(["user", "role"]),
  ...
  .handler(async ({ data }) => {
    await orchestratorJson(
      `/admin/sessions/${data.sessionId}/acl/${data.principalType}/${encodeURIComponent(data.principalId)}`,
      { method: "DELETE" },
    );
    return { ok: true };
  });
